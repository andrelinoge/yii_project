...
class CImageHandler extends CApplicationComponent
{
    ...
    public function load($file)
	{
        ...
        if ($img = $this->loadImage($file))
        {
            ...
            $this->fileName = $file;
            $this->baseFileName = basename( $file );
    ...
    public function cropAndScaleFromCenter( $width, $height )
    {
        $widthOrigin = $this->getWidth();
        $heightOrigin = $this->getHeight();

        $ratioX = $width / $widthOrigin;
        $ratioY = $height / $heightOrigin;

        $ratio       = max( $ratioX, $ratioY );
        $useRatioX = ( $ratioX == $ratio );

        $newWidth   = $useRatioX  ? $width  : floor( $widthOrigin * $ratio );
        $newHeight  = !$useRatioX ? $height : floor( $heightOrigin * $ratio );

        $leftOffset    = $useRatioX  ? 0 : floor( ( $width - $newWidth ) / 2);
        $topOffset     = !$useRatioX ? 0 : floor( ( $height - $newHeight ) / 2);
    ...
    public function squareThumb($to_width, $to_height, $proportional = true)
    {
        $this->checkLoaded();

        if($to_width !== false)
        {
            $to_width = min($to_width, $this->width);
        }

        if($to_height !== false)
        {
            $to_height = min($to_height, $this->height);
        }
        ...
        if ($current_width != $current_height)
        {
            if ($current_width > $current_height)
            {
                $this->crop($current_height, $current_height, ($current_width - $current_height) /2, 0);
            }
            else
            {
                $this->crop($current_width, $current_width, 0, ($current_height - $current_width) /2);
            }
        }

        $this->resize($to_width, $to_height, $proportional);
