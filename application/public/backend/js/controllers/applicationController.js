...
ApplicationController = (function() {
        ...
        $document.on('submit', 'form.ajax-form', function(event) {
            ...
            $form.ajaxSubmit({
                ...
                success: function(response, status, xhr) {
                    ...
                    {
                        _this.show_error_message( response.errorMessage );
                    }

                    // show Success Message if it is setted
                    if( (typeof(response.successMessage) != 'undefined') && (response.successMessage != '') )
                    {
                        _this.show_success_message( response.successMessage );
                    }
    ...
    {
        var $dialog = $('#success-notification');
        $dialog.find('p.content').html(message);

        return $.fancybox.open($dialog);
    };

    ApplicationController.prototype.show_success_message = function(message)
    {
        var $dialog = $('#error-notification');
        $dialog.find('p.content').html(message);
